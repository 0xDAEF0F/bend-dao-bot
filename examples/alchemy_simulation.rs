...
use anyhow::Result;
use ethers::{providers::{Middleware, Provider, Ws}, types::{Address, Bytes, H256, U256}, utils::format_ether};
...
async fn main() -> Result<()> {
    ...
    let provider = Arc::new(Provider::<Ws>::connect("wss://eth-mainnet.g.alchemy.com/v2/S1llhLoNFxJdv4K85HALN0xYqNXaa7d0").await?);
    ...
    for log in logs {
        if &log.topics[0].to_string() == "0x58bdf68b6e757afad014720959e6c9ecd94de1cc24b964ebf48b08b50366b321" {
            println!("New asset price for {:?} of {}", log.topics[1], format_ether(U256::from_str_radix(&log.data.as_str()[..66], 16)?))
        }
    }

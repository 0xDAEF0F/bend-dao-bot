...
use revm::{
    ...
    interpreter::sload,
    primitives::{Address, Bytes, TransactTo, U256 as Uint256},
    Database, DatabaseRef, Evm, JournaledState,
};
...
async fn main() -> Result<()> {
    ...
    let ethers_db = EthersDB::new(
        provider.clone(),
        Some(BlockId::Number(BlockNumber::Number(U64::from(20041297)))),
    )
    .unwrap();
    ...
    while let Some(hash) = stream.next().await {
        let tx = provider
            .get_transaction(H256::from_str(
                "0x5e6fd12eebbd323aa86ab507c89b546e00fed07ca3f13d45a4f0b6fd285b7913",
            )?)
            .await?
            .unwrap();
        // println!("Tx: {:?}", tx);
        ...
        let mut evm = Evm::builder()
            ...
            .modify_tx_env(|env_tx| {
                ...
                env_tx.transact_to =
                    TransactTo::Call(Address::from(tx.to.unwrap().as_fixed_bytes()));
        ...
        for (account_touched, change) in &state {
            for (slot, value) in &change.storage {
                let true_value = provider
                    .get_storage_at(
                        H160::from_str(&account_touched.to_string())?,
                        H256::from_uint(&U256::from_dec_str(&slot.to_string())?),
                        Some(BlockId::Number(BlockNumber::Number(U64::from(20041299)))),
                    )
                    .await?;
                if U256::from_str_radix(&format!("{:?}", true_value), 16)?.to_string()
                    != value.present_value.to_string()
                {
                    println!(
                        "WARINGINGINAS IGN {:?} : {:?}",
                        U256::from_str_radix(&format!("{:?}", true_value), 16)?,
                        value.present_value.to_string()
                    );
                }
        ...
        let tx = provider
            .get_transaction(H256::from_str(
                "0x78e2486ea7808c51524884e2d4389e87d073d252e05aaa30da9c16c7fab5eadc",
            )?)
            .await?
            .unwrap();
        // println!("Tx: {:?}", tx);
            ...
            for (slot, value) in &change.storage {
                let true_value = provider
                    .get_storage_at(
                        H160::from_str(&account_touched.to_string())?,
                        H256::from_uint(&U256::from_dec_str(&slot.to_string())?),
                        Some(BlockId::Number(BlockNumber::Number(U64::from(20043083)))),
                    )
                    .await?;
                if U256::from_str_radix(&format!("{:?}", true_value), 16)?.to_string()
                    != value.present_value.to_string()
        ...
        println!(
            "Time 2: {:?}",
            newer_time.signed_duration_since(current_time)
        );
